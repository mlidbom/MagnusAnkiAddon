    ...
    public static Avalonia.Controls.MenuItem ToAvalonia(MenuItem spec)
    {
        ...
        if (spec.Action != null)
        {
            avaloniaItem.Click += (sender, args) =>
            {
                ...
                {
                    JALogger.Log($"Menu action failed: {ex.Message}\n{ex.StackTrace}");
                    throw; // Re-throw per project exception handling policy
                }
            };
        }
...
{
    public static Avalonia.Input.KeyGesture? Parse(string shortcutText)
    {
        try
        {
            // Avalonia KeyGesture.Parse handles strings like "Ctrl+O", "Ctrl+Shift+S"
            return Avalonia.Input.KeyGesture.Parse(shortcutText);
        }
        catch
        {
            // If parsing fails, just return null - shortcut won't be displayed
