...
public static class VocabNoteSorting
{
        ...
        string? preferredKanji = null)
    {
        ...
        int PreferVocabWithKanji(VocabNote localVocab)
        {
            return (preferredKanji == null || localVocab.GetQuestion().Contains(preferredKanji)) ? 0 : 1;
        }

        int PreferMoreSentences(VocabNote localVocab)
        {
            return -localVocab.Sentences.All().Count;
        }
