...
public sealed class TextAnalysis
{
   ...
   public TextAnalysis(AnalysisServices services, string sentence, SentenceConfiguration sentenceConfiguration, bool forUI = false, string? cachedJanomeTokens = null)
   {
      ...
      IndexingWordVariants = Locations.SelectMany(location => location.IndexingVariants).ToList();
      DisplayWordVariants = Locations.SelectMany(location => location.DisplayVariants).ToList();
      ValidMatches = IndexingWordVariants.SelectMany(variant => variant.ValidMatches).ToList();
      DisplayMatches = DisplayWordVariants.SelectMany(variant => variant.DisplayMatches).ToList();
   }
   ...
   void ConnectNextAndPreviousToLocations()
   {
   ...
   void AnalysisStep1AnalyzeNonCompound()
   {
   ...
   void AnalysisStep2AnalyzeCompounds()
   {
      foreach(var location in Locations)
      {
   ...
   void AnalysisStep3RunDisplayAnalysisWithoutShadowingInformationSoThatAllValidMatchesAreDisplayedAndCanBeAccountedForInYieldingToUpcomingCompoundsReturnTrueOnChanges()
   {
      foreach(var location in Locations)
      {
   ...
   bool AnalysisStep4SetInitialShadowingStateReturnTrueOnChanges()
   {
   ...
   void AnalysisStep5CalculatePreferenceBetweenOverlappingDisplayCandidates()
   {
