...
def test_single_or_none_returns_single_value() -> None: value_test([1], lambda x: x.single_or_default(), 1)
def test_single_or_none_returns_none_if_no_values() -> None: value_test([], lambda x: x.single_or_default(), None)
def test_single_or_none_throws_if_multiple_values() -> None: throws_test([1, 2], lambda x: x.single_or_default(), Exception)
...
def test_for_single_or_none_does_nothing_if_there_are_no_elements() -> None:
    value_test(LList[str]([]), lambda x: x.for_single_or_none(lambda _: 1).to_list(), list[str]())
