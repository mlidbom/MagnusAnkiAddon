...
def reparse_sentences_for_vocab(vocab: VocabNote) -> None:
    ...
    with TaskRunner.current("Reparsing sentences for vocab") as runner:
        sentences: set[SentenceNote] = set(runner.run_on_background_thread_with_spinning_progress_dialog("Fetching sentences to reparse", lambda: app.col().sentences.potentially_matching_vocab(vocab)))
...
def reparse_matching_sentences(question_substring: str) -> None:
    sentences_to_update = app.col().sentences.sentences_with_substring(question_substring)
