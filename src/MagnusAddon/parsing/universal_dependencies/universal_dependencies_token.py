...
class UDToken:
    def __init__(self, token: Union[UDPipeEntry, Token]) -> None:
        ...
        if isinstance(token, Token):
            self.id = typed.int_(token.idx)
            self.deps = typed.str_(token.dep_)
            ...
            self.deprel = ud_relationship_tag.get_tag(typed.str_(token.dep_)) if token.dep_ != _missing_value else None
            self.feats = typed.str_(token.morph)
            self._head_id = typed.int_(token.idx)
