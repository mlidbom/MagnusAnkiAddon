...
def setup_note_menu(note_menu: QMenu, vocab: VocabNote, selection: str, clipboard: str) -> None:
    ...
    def build_toggle_flags_menu(toggle_flags_menu: QMenu) -> None:
        ...
        add_toggle_checkbox(toggle_flags_menu, shortcutfinger.up1("Question overrides form: Show the question in results even if the match was another form"), vocab.matching_rules.question_overrides_form.tag_field)
        add_toggle_checkbox(toggle_flags_menu, shortcutfinger.up2("Requires sentence end: followed by nothing, a non-word, or a quote, character: と、って"), vocab.matching_rules.requires_sentence_end)
