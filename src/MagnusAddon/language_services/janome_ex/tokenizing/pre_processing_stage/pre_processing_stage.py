...
class PreProcessingStage(Slots):
    ...
    def pre_process(self, tokens: list[JNToken]) -> list[ReplacedToken]:
        return [processed for token in tokens for processed in self.pre_process_token(token)]


    def pre_process_token(self, token: JNToken) -> list[ReplacedToken]:  # note: The order here matters, it's not random. any change will break things even should the tests be incomplete and not show it.
