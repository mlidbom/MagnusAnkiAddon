...
class IchidanImperativeSplitter(Slots):
    @classmethod
    def try_split(cls, token: JNToken) -> list[ReplacedToken] | None:
    ...
    @classmethod
    def _split_ichidan_imperative(cls, token: JNToken) -> list[ReplacedToken]:
        ichidan_surface = token.surface[:-1]
        ichidan_imperative_part = token.surface[-1]
        return [ReplacedToken(surface=ichidan_surface, base=token.base_form, is_inflectable_word=True, is_ichidan_imperative_stem=True),
                ReplacedToken(surface=ichidan_imperative_part, base=ichidan_imperative_part, is_inflectable_word=True, is_ichidan_imperative_inflection=True)]
