...
def render_breakdown(note: SentenceNote, replacements:dict[str, str]) ->None:
    ...
    user_higlighted = set(note.get_user_highlighted_vocab())

    breakdown_html = f"""
    <div class="breakdown page_section">
    <div class="page_section_title">breakdown</div>
    {_create_html_from_nodes(view_model.nodes, note.get_user_excluded_vocab(), user_higlighted, 1)}
    </div>
    """
...
def render(note: SentenceNote) -> dict[str, str]:
    ...
    render_breakdown(note, replacements)
