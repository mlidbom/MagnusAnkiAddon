...
def build_string_menu(menu: QMenu, string: str, string_note_menu_factory: Callable[[QMenu, str], None]) -> None:
    ...
    build_create_note_menu(non_optional(menu.addMenu(shortcutfinger.home4(f"Create: {string}"))), string)
    add_ui_action(menu, shortcutfinger.up1("remove from sentence exclusions"), lambda _string=string: local_note_updater.clean_sentence_excluded_word(_string))  # type: ignore
