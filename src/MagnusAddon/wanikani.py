...
from aqt import mw, browser, editor, gui_hooks, deckbrowser, dialogs
...
def get_kanji_dependencies(kanji_note: Note) -> List[Note]:
    radical_names = [item.strip() for item in
                     kanji_note["Radicals_Names"].split(",") + kanji_note["Radicals_Icons_Names"].split(",")]
    radical_note_ids = [mw.col.find_notes("note:radical_Model Radical_Name:" + kanji_name) for kanji_name in
                        radical_names]
...
def get_vocab_dependencies(vocab_note: Note) -> List[Note]:
    kanji_names = [item.strip() for item in vocab_note["Kanji"].split(",")]
    kanji_note_ids = [mw.col.find_notes("note:kanji_Model Kanji:" + kanji_name) for kanji_name in kanji_names]
...
def unsuspend_kanji_with_dependencies(kanji_note: Note) -> None:
    ...
    for radical in radicals:
        print("Unsuspending radical card:" + radical["Radical_Name"])
        mw.col.sched.unsuspend_cards(radical.card_ids())

    print("Unsuspending kanji card:" + kanji_note["Kanji_Meaning"])
...
def unsuspend_vocab_with_dependencies(vocab_note: Note) -> None:
    ...
    for kanji_note in kanji_notes:
        print("Unsuspending kanji card:" + kanji_note["Kanji_Meaning"] + " card:")
        mw.col.sched.unsuspend_cards(kanji_note.card_ids())

    print("Unsuspending vocab card:" + vocab_note["Vocab_Meaning"] + " card:")
