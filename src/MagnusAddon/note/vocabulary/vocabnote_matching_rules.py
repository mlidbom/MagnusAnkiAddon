...
class VocabNoteMatchingRules(Slots):
    ...
    def _add_set[TValue](self, value: QSet[str]):
...
class VocabMatchingRulesConfigurationRequiresForbidsFlags(Slots):
    def __init__(self, vocab: WeakRef[VocabNote]) -> None:
        ...
        self.e_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.e_stem, Tags.Vocab.Matching.Forbids.e_stem)
        self.a_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.a_stem, Tags.Vocab.Matching.Forbids.a_stem)
        self.masu_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.masu_stem, Tags.Vocab.Matching.Forbids.masu_stem)
        self.preceding_adverb: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.preceding_adverb, Tags.Vocab.Matching.Forbids.preceding_adverb)
        self.past_tense_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.past_tense_stem, Tags.Vocab.Matching.Forbids.past_tense_stem)
        self.dictionary_form_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.dictionary_form_stem, Tags.Vocab.Matching.Forbids.dictionary_form_stem)
        self.dictionary_form_prefix: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.dictionary_form_prefix, Tags.Vocab.Matching.Forbids.dictionary_form_prefix)
        self.te_form_stem: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.te_form_stem, Tags.Vocab.Matching.Forbids.te_form_stem)
        self.ichidan_imperative: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.ichidan_imperative, Tags.Vocab.Matching.Forbids.ichidan_imperative)
        self.godan_potential: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.godan_potential, Tags.Vocab.Matching.Forbids.godan_potential)
        self.godan_imperative: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.godan_imperative, Tags.Vocab.Matching.Forbids.godan_imperative)
        self.godan_imperative_prefix: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.godan_imperative_prefix, Tags.Vocab.Matching.Forbids.godan_imperative_prefix)
        self.single_token: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.single_token, Tags.Vocab.Matching.Requires.compound)
        self.sentence_end: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.sentence_end, Tags.Vocab.Matching.Forbids.sentence_end)
        self.sentence_start: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.sentence_start, Tags.Vocab.Matching.Forbids.sentence_start)
        self.exact_match: RequireForbidFlagField = self._add_flag(Tags.Vocab.Matching.Requires.exact_match, Tags.Vocab.Matching.Forbids.exact_match)
    ...
    def _add_flag(self, required_tag: Tag, forbidden_tag: Tag) -> RequireForbidFlagField:
        flag_field = RequireForbidFlagField(self._vocab, required_tag, forbidden_tag)
...
class VocabNoteMatchingConfiguration(WeakRefable, Slots):
    ...
    def has_custom_requirements(self) -> bool:
        return any(flag.is_active for flag in self.requires_forbids.all_flags) or any(field_set.any() for field_set in self._rules().all_sets)
