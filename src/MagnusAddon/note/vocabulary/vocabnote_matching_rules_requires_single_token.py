...
class RequiresSingleToken(TagFlagField, Slots):
    def __init__(self, matching: WeakRef[VocabNoteMatching]) -> None:
        super().__init__(matching().vocab, Tags.Vocab.Matching.Requires.single_token)
        self.matching: WeakRef[VocabNoteMatching] = matching

    def set_to(self, value: bool) -> None:
        super().set_to(value)
        if value: self.matching().requires_compound.set_to(False)
