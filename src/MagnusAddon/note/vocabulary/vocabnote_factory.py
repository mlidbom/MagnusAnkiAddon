...
class VocabNoteFactory(Slots):
    ...
    def create_with_dictionary(question: str) -> VocabNote:
        ...
        readings = dict_entry.entries.select_many(lambda entry: entry.kana_forms_text()).to_list()  #list(set(ex_sequence.flatten([ent.kana_forms() for ent in dict_entry.entries])))
