...
class JPCollection:
    class VocabCollection:
        def __init__(self, collection: JPCollection):
            self.collection = collection

        def search(self, query: str) -> list[VocabNote]:
            return [VocabNote(note) for note in (self.collection._search_notes(query))]

        def by_vocab(self, field_values: list[str]) -> List[VocabNote]:
            notes = self.collection._fetch_notes_by_note_type_and_field_value(NoteTypes.Vocab, NoteFields.Kanji.question, field_values)
            vocab_notes = [VocabNote(note) for note in notes]
            return vocab_notes

        def all_wani(self) -> List[VocabNote]:
            notes = self.collection._fetch_notes_by_note_type(NoteTypes.Vocab)
            vocab_notes = [VocabNote(note) for note in notes]
    ...
    class KanjiCollection:
        ...
        def search(self, query: str) -> list[KanjiNote]:
            return [KanjiNote(note) for note in self.collection._search_notes(query)]

        def all(self) -> List[KanjiNote]:
            notes = self.collection._fetch_notes_by_note_type(NoteTypes.Kanji)
        ...
        def by_kanji(self, field_values: list[str]) -> List[KanjiNote]:
            notes = self.collection._fetch_notes_by_note_type_and_field_value(NoteTypes.Kanji, NoteFields.Kanji.question, field_values)
            kanji_notes = [KanjiNote(note) for note in notes]
            return kanji_notes
    ...
    class RadicalCollection:
        ...
        def by_answer(self, field_values: list[str]) -> List[RadicalNote]:
            notes = self.collection._fetch_notes_by_note_type_and_field_value(NoteTypes.Radical, NoteFields.Radical.answer, field_values)
            radical_notes = [RadicalNote(note) for note in notes]
            return radical_notes

        def all(self) -> List[RadicalNote]:
            notes = self.collection._fetch_notes_by_note_type(NoteTypes.Radical)
            radical_notes = [RadicalNote(note) for note in notes]
            return radical_notes
    ...
    class SentenceCollection:
        ...
        def all(self) -> list[SentenceNote]:
            return [SentenceNote(note) for note in self.collection._fetch_notes_by_note_type(NoteTypes.Sentence)]

        def search(self, query: str) -> list[SentenceNote]:
            return [SentenceNote(note) for note in self.collection._search_notes(query)]


    def __init__(self, anki_collection: Collection):
        self.anki_collection = anki_collection
        self.vocab = JPCollection.VocabCollection(self)
        self.kanji = JPCollection.KanjiCollection(self)
        self.sentences = JPCollection.SentenceCollection(self)
        self.radicals = JPCollection.RadicalCollection(self)

    def _fetch_notes_by_note_type_and_field_value(self, note_type: str, field: str,
        ...
        notes = self._fetch_notes_by_id(note_ids_flat)
        return notes

    def _fetch_notes_by_note_type(self, note_type: str) -> List[anki.notes.Note]:
    ...
    def _fetch_notes_by_id(self, note_ids: Sequence[NoteId]) -> List[anki.notes.Note]:
