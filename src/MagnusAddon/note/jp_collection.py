...
class JPCollection:
    ...
    def list_sentence_notes() -> list[SentenceNote]:
        return [SentenceNote(note) for note in JPCollection.fetch_notes_by_note_type(NoteTypes.Sentence)]
    ...
    def fetch_notes_by_note_type(note_type: str) -> List[anki.notes.Note]:
        notes = JPCollection._search_notes("{}:{}".format(Builtin.Note, note_type))
        return notes
    ...
    def fetch_kanji_notes(field_values: List) -> List[KanjiNote]:
        notes = JPCollection.fetch_notes_by_note_type_and_field_value(NoteTypes.Kanji, NoteFields.Kanji.question,
                                                                      field_values)
    ...
    def fetch_radical_notes(field_values: List) -> List[RadicalNote]:
        notes = JPCollection.fetch_notes_by_note_type_and_field_value(NoteTypes.Radical,
    ...
    def fetch_vocab_notes(field_values: List) -> List[VocabNote]:
        notes = JPCollection.fetch_notes_by_note_type_and_field_value(NoteTypes.Vocab, NoteFields.Kanji.question,
                                                                      field_values)
    ...
    def fetch_all_radical_notes() -> List[RadicalNote]:
        notes = JPCollection.fetch_notes_by_note_type(NoteTypes.Radical)
        radical_notes = [RadicalNote(note) for note in notes]
        return radical_notes

    @staticmethod
    def fetch_all_kanji_notes() -> List[KanjiNote]:
        notes = JPCollection.fetch_notes_by_note_type(NoteTypes.Kanji)
        kanji_notes = [KanjiNote(note) for note in notes]
        return kanji_notes

    @staticmethod
    def fetch_all_wani_kanji_notes() -> list[KanjiNote]:
        return [kanji for kanji in JPCollection.fetch_all_kanji_notes() if kanji.is_wani_note()]

    @staticmethod
    def fetch_all_wani_vocab_notes() -> List[VocabNote]:
        notes = JPCollection.fetch_notes_by_note_type(NoteTypes.Vocab)
        vocab_notes = [VocabNote(note) for note in notes]
