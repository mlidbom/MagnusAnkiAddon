...
class SentenceQuestionField(Slots):
    ...
    def get(self) -> str: return ex_str.strip_html_markup(self._get_raw().replace(self._word_break_tag, ex_str.invisible_space))

    def split_token_with_word_break_tag(self, section: str) -> None:
        if len(section) < 2: return
        new_section = f"{section[0]}{self._word_break_tag}{section[1:]}"
